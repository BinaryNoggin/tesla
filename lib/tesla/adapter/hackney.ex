if Code.ensure_loaded?(:hackney) do
  defmodule Tesla.Adapter.Hackney do
    ...
    defp handle({:ok, ref}) when is_reference(ref) do
      async_status({ref, %{status: nil, headers: nil}})
    end
    ...
    defp async_status({ref, %{headers: headers, status: status}})
    when not (is_nil(headers) or is_nil(status)) do
      {:ok, status, headers, ref}
    end

    defp async_status({ref, output}) do
      receive do
        {:hackney_response, ^ref, {:status, status, _}} ->
           response_status({ref, %{output | status: status}})
        {:hackney_response, ^ref, {:headers, headers}} ->
          response_status({ref, %{output | headers: headers}})
      end
    end
  end
end
